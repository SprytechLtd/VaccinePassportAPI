...
module.exports = {
    encodeFileToBase64: async function (file) {
        let base64;
        try {
            console.log('in encodeFileToBase64');
    ...
    fileUpdate: async function (fileData, fileId) {
        const privateKey = PrivateKey.fromString(process.env.TREASURY_ACCOUNT_ID);
    ...
    fileCreate: async function (fileData) {
    ...
    convertToJSONFile: async function (fileJSON) {
        try {
            fs.writeFile(path.join(__dirname, '/jsonFiles/', 'file.json'), JSON.stringify(fileJSON), (error,content) => {
                if (error) {
                    console.log('error at uploadFile method')
                    console.log(err.message);
                    return {};
                } else {
                    console.log('file created and converted to json');
                    return content;
                }
            });
            //return fileData
    ...
    loadJsonFile: async function (filePath) {
        try {
            ...
            fs.readFile(filePath, JSON.stringify(fileJSON), (error,content) => {
                if (error) {
                    console.log('error at uploadFile method')
                    console.log(err.message);
                    return {};
    ...
    fileGetContents : async function (fileId) {
        const client = tokenServiceModule.hederaClientLocal(process.env.TREASURY_ACCOUNT_ID, process.env.TREASURY_PRIVATE_KEY);
        let info = {};
        try {
