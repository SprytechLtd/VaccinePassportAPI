...
const upload = multer()
var fs = require('fs');
const tfileServiceeModule = require('./fileService')
...
router.post('/patientRegistration', upload.single('national_id'),  async (req, res) => {
    ...
    if(req.file) {
        try{
          const base64 = await tfileServiceeModule.encodeFileToBase64(req.file)
          const resultfileId = await tfileServiceeModule.fileUpdate(JSON.stringify(base64), fileId);
