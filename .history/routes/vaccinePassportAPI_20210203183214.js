...
router.post('/patientRegistration', upload.single('national_id'), async (req, res) => {
    ...
    try {
        ...
        const fileData = await fileServiceModule.convertToJSONFile(patientDets);
       ...
       // let patientFile = fileServiceModule.loadJsonFile(patientFilePath);
        ...
        if (patientFile) {
            try {
                const base64 = await fileServiceModule.encodeFileToBase64(getStream)
        ...
        const fileData = await fileServiceModule.convertToJSONFile("");
        console.log(error);
