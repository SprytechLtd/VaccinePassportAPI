  ...
  if(mobile.length === 0 || username.length === 0 || email.length === 0  || email.password === 0 ) {
    ...
    if(!errors){
      ...
      var form_data = {
        ...
        email: email,
        mobile: mobile,
        privateKey : newAccountPublicKey.toString(),
        publicKey : newAccountPrivateKey.toString(),
        ...
        mysqlConnection.query('INSERT INTO passport_users SET ?', form_data, function(err, result) {
          ...
          if (err) {
            ...
            output["status"] = true
            output["id"] = result.insertIdpublicKey
            output["name"] =  username
            output["email"] =  email
            output["mobile"] =  mobile
            output['hederaAccount'] =  { "accountId" : newAccountId.toString(), "publicKey" : newAccountPrivateKey.toString(), "privateKey" : newAccountPublicKey.toString }
            res.send(output)
